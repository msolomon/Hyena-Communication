...
void pop::save_data(int g,int trial){
	...
	data[2+trial*(TEAM_SIZE*2+2)][g] = the_pop[best]->get_avg_dist_to_zebra();
	data[3+trial*(TEAM_SIZE*2+2)][g] = the_pop[best]->get_avg_lion_attacks();
	for(int t = 0; t< TEAM_SIZE;t++){
		data[4+t+trial*(TEAM_SIZE*2+2)][g] = the_pop[best]->get_scout_fit(t);
		data[4+t+TEAM_SIZE+trial*(TEAM_SIZE*2+2)][g] = the_pop[best]->get_invest_fit(t);
	}
	pop_bestteam = best;
...
void pop::OET1_reproduce(){
	...
	for(int i=0; i < NUM_LIONS; i++){          
		...
		}while(parent_investigator == team_replace1 || parent_investigator == team_replace2);  
		the_pop[team_replace1]->copy(the_pop[parent_investigator],i,investigator);  
		do{  // second parent's investigator
			parent_investigator = member_select(1,i,investigator);  
		}while(parent_investigator == team_replace1 || parent_investigator == team_replace2);  
		the_pop[team_replace2]->copy(the_pop[parent_investigator],i,investigator);    
	}      
...
void pop::oet_generational(){
	...
	for(int i = POP_SIZE/2; i < POP_SIZE; i=i+2){
		...
		for(int j=0; j < NUM_LIONS; j++){
			...
			parent_investigator = member_select(1,j,investigator);  
			temp.the_pop[i+1]->copy(the_pop[parent_investigator],j,investigator);    
		}      
		temp.the_pop[i]->calc_size();
		temp.the_pop[i+1]->calc_size();
		temp.the_pop[i]->xOver(temp.the_pop[i+1]);
