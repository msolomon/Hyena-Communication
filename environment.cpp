...
void environment::evaluate(void){
	...
	for(int i = 0; i < NUM_HYENAS; i++){
		...
		for(int j = 0; j < NUM_LIONS; j++){    
			radius = sqrt( pow((tempx-agents->invests[j].getX()),2) +  pow((tempy-agents->invests[j].getY()),2) );
...
void environment::update_vectors(void){
	...
	for(int i = 0; i < NUM_HYENAS; i++){
		...
		for(int j = 0; j < NUM_HYENAS; j++){
			magnitude = sqrt( pow((agentx-agents->scouts[j].getX()),2) + pow((agenty-agents->scouts[j].getY()),2));
			if(magnitude < min_mag && i!= j){
				min_mag=magnitude;
				direction = atan2(agentx-agents->scouts[j].getX(),(agenty-agents->scouts[j].getY()));
		...
		for(int j = 0; j < NUM_HYENAS; j++){
			magnitude = sqrt( pow((agentx-agents->scouts[j].getX()),2) + pow((agenty-agents->scouts[j].getY()),2));
			if(agents->scouts[j].get_calling() == true && magnitude < min_mag && i!= j && magnitude < X){
				min_mag=magnitude;
				direction = atan2(agentx-agents->scouts[j].getX(),(agenty-agents->scouts[j].getY()));
			}
		}
		...
		for(int j = 0; j < NUM_LIONS; j++){
			magnitude = sqrt( pow((agentx-agents->invests[j].getX()),2) + pow((agenty-agents->invests[j].getY()),2));
			if(magnitude < min_mag && magnitude < X/4){
				min_mag=magnitude;
				direction = atan2(agentx-agents->invests[j].getX(),(agenty-agents->invests[j].getY()));
			}
		}
		...
		//zebra     
		temp.magnitude = sqrt( pow(double(agentx-ZEBRAX),2.0) + pow(double(agenty-ZEBRAY),2.0));
	...
	for(int i = 0; i < NUM_LIONS; i++){
		...
		// zebra
		temp.magnitude = sqrt( pow((agentx-ZEBRAX),2) + pow((agenty-ZEBRAY),2));
		temp.direction = atan2(agentx-ZEBRAX,agenty-ZEBRAY);
...
void environment::draw(QPointer<GLWidget> wid){
	...
	for(int i = 0; i < NUM_HYENAS; i++){
		wid->helper->hyenas[i] = QPoint(agents->scouts[i].getX()*BOX, agents->scouts[i].getY()*BOX);
	}
}
